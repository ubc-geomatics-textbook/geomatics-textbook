...
**Phenomenon**, noun, plural *Phenomena*: *1* a fact or situation that is observed to exist or happen, especially one whose cause or explanation is in question. *2* a remarkable person, thing, or event [Oxford's Dictionary](https://www.google.com/search?channel=trow5&client=firefox-b-d&q=phenomena).  Essentially, anything and everything are phenomenon: lightning, a country, coastlines, a dog on a kayak.  Broadly speaking, in GIS we categorize phenomena as **discrete** or **continuous**.  Both kinds of phenomena can be represented in a GIS, but they come with different considerations and cannot always be represented with the same kind of data model.

```{r 3-dog-on-a-boat, echo=FALSE, out.width = '75%', fig.cap=fig_cap}
fig_cap <- paste0("Yarrow enjoying the scenery at Aloutte Lake, she's quite the phenomenon indeed. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Provinces are clearly delineated, distinct objects, despite having no real physical presence. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Elevation of the Sunshine Coast in BC, from the NRCAN DEM (google earth engine). Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("The west coast of Vancouver Island (just a screen grab from Arc, might want something better. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Global lighting strike density per month. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Summary of the types of data. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Each flower is different, but no flower is 'more' or 'less' a flower than any of the others. Pickell, CC-BY-SA-4.0.")
knitr::include_graphics("images/03-flowers-nominal.png")
```

```{r 3-nominal-opperations, echo=FALSE, out.width = '75%', fig.cap=fig_cap}
fig_cap <- paste0("Checking equality with flower species. Pickell, CC-BY-SA-4.0.")
knitr::include_graphics("images/03-nominal-opperations.png")
```
...
fig_cap <- paste0("We can see Yarrow is taller than her sister Shamsa, so we can rank these dogs by height.  However, we haven't measured their heights, so we don't know how much taller Yarrow is than Shamsa. Pickell, CC-BY-SA-4.0.")
...
```{r 3-ordinal-operatons, echo=FALSE, out.width = '75%', fig.cap=fig_cap}
...
knitr::include_graphics("images/03-ordinal-operatons.png")
```
...
fig_cap <- paste0("In this example, we see an alpine landscape in Garibalid Provinical Park, BC.  We can see patches of forest and patches of meadow.  But where, exactly, would we draw the boundary between these two landscape classes. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Because of the fixed, meaningful zero point, we can calculate ratios between populations: e.g. Manitoba's population is 1/10th that of Ontario, British Columbia has 129 times as many people as Nunavut. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("The ratio between two temperatures in Celsius is not meaningful, 20&deg;C is not 'twice' as warm as 10&deg;C. Kelvin's zero point is fixed to absolute zero, the 'absence' of temperature.  So we can calculate the ratio, 293.15&deg;K is 1.035 times warmer than 283.15&deg;K. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Income vs. household expenditures on Food by Census Subdivisions in BC. Pickell, CC-BY-SA-4.0.")
knitr::include_graphics("images/03-income-v-food.png")
```

```{r 3-income-v-income-on-food, echo=FALSE, out.width = '75%', fig.cap=fig_cap}
fig_cap <- paste0("Income vs. Fraction of Income Spent on Food by Census Subdivisions in BC. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Visualization of Tobler's First Law. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Screengrab from Hectares BC, can easily make into better map. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Representing space in the raster model vs. the vector model [8]. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("I think this one is pretty good, I use it in my lecture notes[8]. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Raster math illustration. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Raster Resolution. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Vector objects (points, lines, or polygons) are stored along with any number of attribute.  Point, line, and polygon data are typically stored in separate files. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("An example of point data showing locations of trees.  The points are labeled with their index (unique ID number) which correspoonds to the attribute table below which stores more information about eacht tree. Pickell, CC-BY-SA-4.0.")
...
fig_cap <- paste0("Vector image of Nova Scotia at different resolutions.  Here the original polygon (top left) has been downsampled to lower resolutions, by setting the minimum allowable distance between verticies.  As the distance beween vercicies increases, the resolution decreases and the coastline becomes less distinguishable. Pickell, CC-BY-SA-4.0.")
...
#### Resolution: Vector vs. Raster {#box-text -}
