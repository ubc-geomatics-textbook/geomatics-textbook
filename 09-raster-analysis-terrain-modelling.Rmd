...
Natural processes on a landscape can be interpreted by analyzing its topography. DEMs can be used to create a wide range of topographic indices, which may be used to inform on landscape geomorphology, hydrology and biological processes in a study area (Mattivi et al., 2019). To demonstrate the applications of topographic indices, and attempt was made to predict the distribution of wetlands across a study area in the Fort St. John Timber Supply Area, in North-eastern British Columbia, Canada (Figure \@ref(fig:9-fort-st-john-study-area)).
...
fig_cap <- "The Fort St. John Timber Supply Area & Chowade Watershed Management Basin, British Columbia, Canada. Data from Government of British Columbia 2011a and 2011b. Mesler, CC-BY-SA-4.0."
m
```
...
```{r 9-SAGA-Wetness, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste("System for Automated Geoscientific Analyses (SAGA) Wetness Index across the Chowade Watershed. Data from Government of British Columbia 2011b. Melser, CC-BY-SA-4.0.")
    knitr::include_graphics(here::here("images", "09-SAGA-Wetness.png"))
```

In addition to SAGA Wetness, another useful index that was included in the Case Study was the Multi-Resolution Valley Bottom Flatness (MRVBF) index. Introduced by Gallant and Dowling in 2003, this index identifies hill slopes and valley bottoms, and characterizes hydrological and geomorphic dynamics. This index serves to identify areas that are likely to be occupied by water, and has been used widely in remote sensing studies (Huang et al. 2017). MRVBF is derived from flatness and lowness characteristics and is computed using slope and ranked elevation, see Figure \@ref(fig:9-MRVBF-diagram) (Gallant and Dowling 2003). Note that this is a simplification of the computational process, and Gallant and Dowling (2003) may be consulted for a more in-depth breakdown of the 18 equations involved in computing MRVBF. Figure \@ref(fig:9-MRVBF) shows MRVBF across the study site.

```{r 9-MRVBF-diagram, echo=FALSE, out.width = '75%', fig.cap=fig_cap}
fig_cap <- paste("Multi-Resolution Valley Bottom Flatness (MRVBF) Computational Components described by Gallant and Dowling 2003. Data from Government of British Columbia 2011b. Melser, CC BY 4.0.")
