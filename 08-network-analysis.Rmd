...
node_names <- unique(c(unique(origins), unique(destinations)))

groups <- c(1, 2, 2, 1, 1, 3, 1, 2, 2, 2, 2)
links <- data.frame(source=origins, target=destinations, value=1)
nodes <- data.frame(name=node_names, group=groups)
...
nodes <- readOGR("data/fraser_river_nodes.geojson", verbose=FALSE)
links <- readOGR("data/fraser_river.geojson", verbose=FALSE)
...
### Routing
Finding routes between nodes is an useful application of spatial networks. This is how maping tools help us navigate the world by finding the most efficient route to move around the city, for example. [@systemsinnovationNetworkDiffusionContagion2015].
...
nodes <- readOGR("data/shortest_path_nodes.geojson", verbose=FALSE)
path <- readOGR("data/shortest_path.geojson", verbose=FALSE)
...
origin <- readOGR("data/pacific_spirit_origin.geojson", verbose=FALSE)
base_net <- readOGR("data/pacific_spirit_network.geojson", verbose=FALSE)
net_400 <- readOGR("data/pacific_spirit_400.geojson", verbose=FALSE)
net_800 <- readOGR("data/pacific_spirit_800.geojson", verbose=FALSE)
net_1200 <- readOGR("data/pacific_spirit_1200.geojson", verbose=FALSE)
net_1600 <- readOGR("data/pacific_spirit_1600.geojson", verbose=FALSE)

leaflet(net_1600, height=500, width="80%") %>% setView(lng=-123.22165125449455, lat=49.25752614491611, zoom=14) %>% addTiles() %>% 
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolylines(data=base_net, color="gray") %>%
  addPolylines(data=net_1600, color="blue") %>%
  addPolylines(data=net_1200, color="green") %>%
  addPolylines(data=net_800, color="yellow") %>%
  addPolylines(data=net_400, color="red") %>%
...
intersections <- readOGR("data/ubc_intersections.geojson", verbose=FALSE)
...
segments <- readOGR("data/ubc_segments.geojson", verbose=FALSE)
...
parks <- readOGR("data/vancouver_parks.geojson", verbose=FALSE)
...
ggplot(parks_df, aes(x=parks.closeness_mean)) + geom_density(alpha=0.2, fill="#FF6666")
