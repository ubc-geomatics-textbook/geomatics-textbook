...
Geodesy, Datum, Ellipsoid, Spheroid, Geoid, Elevation, Orthometric Height, Geoid Height, Geodetic Height, Coordinate System, Celestial Coordinate System, Cartesian Coordinate System, Geographic Coordinate System, Projected Coordinate System, Map Projection
...
[add figure of ellipsoid defining the two axes]
...
[add image/illustration showing the above regional datums fitting over Earth]
...
[insert figure here showing relationship between H, N, and h] 
...
[Figure 2.X. Sinusoidal map projection]
...
[Figure 2.X. Albers map projection]

#### Azimuthal

Azimuthal projections use a flat circular plane to project the Earth onto a map. This plane is usually oriented so that it is tangent at a single point, usually the North or South Pole. In the case of a polar azimuthal projection, meridians radiate outward as straight lines from the pole to the edge of the circular plane and the parallels are represented as concentric circles. As a result, distortion increases as you move away from the centre point of the map with the outer edge of the plane representing an **antipode** or an opposite point on Earth. The primary benefit of azimuthal projections is that they preserve direction and distance between the centre point and any other point in the map. Thus, the shortest geographic distance between the centre point and any other point, known as a **great circle**, can be mapped as a straight line. Azimuthal projections are commonly used when distance and direction are important, such as weather RADAR stations or air traffic control towers. It is important to realize that the centre point for an azimuthal projection can be any point on Earth and the equidistant property can be exploited for a number of applications like mapping circular field plots.

[Figure 2.X. Azimuthal map projection]

### Measuring map projection distortion

In this next section, we will look at how distortions from some different map projections

``` {r echo=F, include=F}
library(rgdal)
library(leaflet)
countries <- rgdal::readOGR("canada.geojson")
...
```{r echo=FALSE}
#htmltools::tagList(
#  htmltools::tags$script(src = "projection-app.min.js"),
#  htmltools::tags$script(src = "world-110m.json"),
#)
```
