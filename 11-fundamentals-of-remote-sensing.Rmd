...
thermal and electromagnetic. Sonic, or sound, **energy** is commonly
used in environments where heat or light are inconsistent or hard to
measure. Sonar is as example of a sonic remote sensing technique and is
often deployed to observe objects in liquid. Thermal, or heat,
**energy** is used in a variety of cases when changes in temperature
indicate an underlying physiological process. An environmental example
would be the observation of plant health via leaf temperature. Although
both sonic and thermal energies provide useful information across a
variety of disciplines, they are not the most common.

Electromagnetic **radiation** (EMR), which is what the human sense of
sight observes, is a popular **energy** source in remote sensing
science. At it's most basic, this method of observation is based on the
measurement of **photons**. Research using EMR is fundamentally
interested in examining the interactions of EMR, or photons, and other
particles. Before diving into the applications of EMR remote sensing
(photography, spectroscopy, etc.) it is important to understand some
basic theory regarding the measurement of photons.
...
through the use of an equation. Albert Einstein explained the **energy**
of a photon as the product of its wave frequency (the number of waves
that pass a specific point over a certain amount of time) and Planck's
...
frequency results in an increase of **energy**. This equation may be
altered to include other properties, such as the speed of light and
wavelength (Equation \@ref(eq:energy-wvl)).
...
body (\~310 K) are around 10^-4^. These measurements are theoretical and
...
same temperature. It is important to remember, however, that blackbodies
do not exist and any real object will always emit a temperature less
than a theoretical blackbody.
...
electromagnetic **energy** that contacts our 1 m^-2^ surface, which
...
which photons an optical sensor *could* record in naturally illuminated
...
1.  The number of zeros in 2.5 x 10-7

2.  The **energy** (E) of a photon with a wavelength of 450 nm.

3.  The absorptance value for a surface with a reflectance of 0.4 and a
    transmittance of 0.35
:::
...
```{r 11-specsig, fig.cap = fig_cap, out.width = "200%", echo = FALSE}
    fig_cap <- paste("Five spectral signatures of various living and non-living samples collected using an ASD FieldSpec3 Imaging Spectroradiometer.")
...
sole purpose of observing the Earth. Landsat is NASA suite of satellites
...
[@raney1991]. It's active nature also enables RADARsat to record
...
#### Optical remote sensing to evaluate land cover in Canada {.unnumbered}
...
scales. These studies can be limited, however, by the resolutions at
which information is collected. Hermosilla et al., 2018 provide an
interesting method of land cover assessment that utilizes Landsat to
derive land cover and land cover change across Canada over a 29 year
span [@hermosilla2018].

Before exploring the methods of this study, it is important to highlight
the qualities of the Landsat program that enabled it. Launched in 1973,
...
introduced since the 1973 launch, each Landsat satellite has a spatial
resolution of 30m^2^. Considering the spatial coverage of this mission
includes the majority of our planet, this spatial resolution is quite
useful.
