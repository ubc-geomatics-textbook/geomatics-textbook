...
Absorption, Aerial, Biconcave, Biconvex, Convex, Concave, Curvature, Focus, Geosynchronous Equitorial Orbit, Hyperspectral, LiDAR, Low Earth Orbit, Multispectral, Nadir, Near-Polar Orbit, Oblique, Orbit, Panchromatic, Radiometric, Reflection, Refraction, Resolution, Spectral, Sun-Synchronous Orbit, Thermal
...
fig_cap <- paste0("Measuring the radius of curvature for a convex optical surface. Claire Armour, CC-BY-SA-4.0")
knitr::include_graphics(here::here("images", "12-ROC_convex.png"))
```
<br/>

```{r 12-RoC-concave, fig.cap = fig_cap, out.width="90%", echo = FALSE}
fig_cap <- paste0("Measuring the radius of curvature for a concave optical surface. Claire Armour, CC-BY-SA-4.0")
...
fig_cap <- paste0("Measurements in a biconvex lens. [DrBob](https://en.wikipedia.org/wiki/User:DrBob), CC-BY-3.0 Unported.")
...
fig_cap <- paste0("Measurements in a biconcave lens. [DrBob](https://en.wikipedia.org/wiki/User:DrBob), CC-BY-3.0 Unported.")
...
fig_cap <- paste0("Mt Assiniboine, image one. Kyle Maguire, CC-BY-SA-4.0")
...
fig_cap <- paste0("Mt Assiniboine, image two. Kyle Maguire, CC-BY-SA-4.0")
...
fig_cap <- paste0("Aerial photo of forest, road, and river near Kitimat, British Columbia. [Ben den Engelsen](https://unsplash.com/@benjeeeman) on Unsplash. [Unsplash License.](https://unsplash.com/license)")
...
fig_cap <- paste0("How aerial imagery should be taken. <a href='",baseurl,"#fig:12-aerial-good'> Animated figure can be viewed in the web browser version of the textbook</a>. Claire Armour, CC-BY-SA-4.0")
knitr::include_graphics(here::here("images", "12-aerial-good.gif"))
```

```{r 12-aerial-bad, fig.cap = fig_cap, out.width="90%", echo = FALSE}
fig_cap <- paste0("How aerial imagery should NOT be taken. <a href='",baseurl,"#fig:12-aerial-bad'>Animated figure can be viewed in the web browser version of the textbook</a> Claire Armou, CC-BY-SA-4.0")
...
fig_cap <- paste0("Looking towards the sky through the trees in a forest in Sweden. [Joachim Bohlander](https://unsplash.com/@joacimbohlander) on Unsplash. [Unsplash License.](https://unsplash.com/license)")
...
If the plane of the lens is _not_ perpendicular to the ground plane, then the imagery is considered **oblique**. Oblique imagery is ideally suited for comparing object sizes or viewing areas that would be otherwise occluded in aerial imagery. Nearly all terrestrial platforms take oblique imagery and it is readily used for airborne and spaceborne platforms. A scanning platform will have an oblique perspective when it is not at the nadir or zenith of its scan arc. Figure \@ref(fig:12-oblique-louise) is an example of an oblique image of a natural area.

```{r 12-oblique-louise, fig.cap = fig_cap, out.width="90%", echo = FALSE}
fig_cap <- paste0("Oblique image of mountainside trees in Lake Louise, Alberta, Canada. [Vlad Shapochnikov](https://unsplash.com/@vladshap) on Unsplash. [Unsplash License.](https://unsplash.com/license)")
...
fig_cap <- paste0("Visualization of a hemispherical perspective. Claire Armour, CC-BY-SA-4.0")
...
fig_cap <- paste0("Hemispherical photo taken in the Bavarian forest. [Martin Wegmann](https://commons.wikimedia.org/wiki/User:Wegmann), CC-BY-3.0 Unported.")
...
fig_cap <- paste0("Picture of Newfoundland, Canada, taken by David Saint-Jacques during his space mission. [Canadian Space Agency/NASA](https://asc-csa.gc.ca/eng/terms.asp#copyright), CC-BY-3.0 Unported.")
...
fig_cap <- paste0("A 360 degree camera mounted on a vehicle is used for collecting street view imagery for Google™ Maps. [Gordon Leggett](https://commons.wikimedia.org/wiki/User:GRDN711), CC-BY-SA-4.0")
...
fig_cap <- paste0("Time lapse from a camera mounted on a tree. Each image is taken on a different day at noon. <a href='",baseurl,"#12-phenological-camera-sequence'>Animated figure can be viewed in the web browser version of the textbook.</a> Christopher Bater, CC-BY-SA-4.0")
...
fig_cap <- paste0("AMSPEC radiometer affixed to a carbon flux tower located in Buckley Bay, Vancouver Island, Canada.  Photo by Nicholas Coops, used with permission.")
...
Consider Newton's cannonball thought experiment: you fire a cannonball perpendicular to Earth's surface at the top of a tall mountain. If the cannonball is traveling at 0 $km·s^{-2}$, then the cannonball falls to the ground due to the force of gravity. If the cannonball is traveling at 1 $km·s^{-2}$, then the cannonball travels some distance over an arc before it eventually falls back to Earth due to gravity. If the cannonball is traveling at least 7.8 $km·s^{-2}$, then the speed of the cannonball is roughly equivalent to the force of gravity that is trying to pull the cannonball back to Earth and therefore the cannonball maintains an elliptical orbit around Earth. In other words, the velocity of the  cannonball is faster over Earth than it is falling towards Earth, which results in a curved path or orbit. 
...
<p id="box-text">
Earth is an oblate ellipsoid with a slightly shorter axis of rotation than the equatorial axis, so a polar orbiting satellite can vary in "altitude" by as much as 30 km.
</p>
