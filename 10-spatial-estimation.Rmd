...
``` {r 10-load-packages-and-setup, echo = FALSE, include = FALSE}
...
For example, when we take another sample of 60 plots as in (Figure \@ref(fig:10-simple-random-sampling)), we can see that different **sampling units (plots)** are being selected from what we obtained in (Figure \@ref(fig:10-simple-random-sampling)), this represent the **equal probability** of each sampling unit being selected.

```{r 10-simple-random-sampling, echo=FALSE, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-simple-random-sampling-2)) Simple random sample of 60 units from a population of 625 units.

```{r 10-simple-random-sampling-2, echo=FALSE,warning=FALSE}
m <- matrix(c(1:625), nrow=25, ncol=25)
...
(Figure \@ref(fig:10-simple-random-sampling-2)) Another simple random sample of 60 units.
...
For example, a forest under study is divided **(stratified)** into similar  regions  (Figure \@ref(fig:10-stratified-random-sampling)) defined by elevation, soil moisture, and soil nutrient gradient and random samples are taken within each strata. The stratification of a study region despite of its size can help to spread the sample over the entire study area.


```{r 10-stratified-random-sampling, echo=FALSE, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-stratified-random-sampling-2)) Stratified random sample within unequal strata within a study area.

```{r 10-stratified-random-sampling-2, echo=FALSE,warning=FALSE}
m <- matrix(c(1:400), nrow=20, ncol=20)
df <- expand.grid(x=1:ncol(m),y=1:nrow(m))
df$val <- m[as.matrix(df[c('y','x')])]
...
(Figure \@ref(fig:10-stratified-random-sampling-2)) Stratified random sample from equal strata within a study area.
...
```{r 10-systematic-sampling, echo=FALSE,warning=FALSE,message=FALSE}
m <- matrix(c(1:400), nrow=20, ncol=20)
...
(Figure \@ref(fig:10-systematic-sampling-2)) Sample every second observation in the row 

```{r 10-systematic-sampling-2, echo=FALSE,warning=FALSE}
m <- matrix(c(1:400), nrow=20, ncol=20)
...
p
```
(Figure \@ref(fig:10-systematic-sampling-2)) Sample all the observation in every second column 
...
```{r 10-cluster-sampling, echo=FALSE, warning=FALSE, message=FALSE}
m <- matrix(c(1:400), nrow=20, ncol=20)
df <- expand.grid(x=1:ncol(m),y=1:nrow(m))
df$val <- m[as.matrix(df[c('y','x')])]
...
(Figure \@ref(fig:10-cluster-sampling)) Cluster of plots selected from entire study area
...
```{r 10-spatial-autocorrelation, echo=FALSE, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-spatial-autocorrelation)) Example of a positive (left) and negative spatial autocorrelation (right) for a give domain.
...
```{r 10-morans-I, echo=FALSE, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-morans-I)) CMI plots location for williams lake TSA and 100 miles house TSA. Basal area has been calculated using the sum of the basal area for each plot (dot) over the given polygon.
:::
...
```{r 10-using-contiguity, echo=FALSE, Message=FALSE, Warinig=FALSE}
...
(Figure \@ref(fig:10-using-contiguity)) Rook's  (left) and Queen's (right) case for searching the neighborhood (grey unit) for the darker unit in the center.
...
Nugget (C): Nugget refers to an unaccounted autocorrelation due to a small lag distance than sampling distance or due to sampling errors (Figure \@ref(fig:10-gearys-c-2)).

Range (R): The distance at which a variogram model first flattens out. This is the distance upto which y variables are spatially autocorrelated or aggregated (Figure \@ref(fig:10-gearys-c-2)).

Sill (S): The value of semivariance that a variogram model attains at a given range is called sill (Figure \@ref(fig:10-gearys-c-2)).
...
```{r 10-semivariogram-modelling, echo=FALSE, warning=FALSE, message=FALSE}
data(fulmar)
...
(Figure \@ref(fig:10-semivariogram-modelling)) An example semivariogram with all the components using the "Fulmar" [see details @Pebesma2005] data from "gstat" package in R.
...
![(Figure \@ref(fig:10-semivariogram-modelling)). Location of Fort Saint Johns TSA and the young stand change monitoring plots.](images/10-FSJ-plot.png){.center}
...
```{r 10-semivariogram-modelling-load-data-and-setup, echo=F, message=F, warning=F}
data<-read.csv("data/10/FSJ.csv",header=T)
```


```{r 10-gaussian, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-gaussian)) A semivariogram using the Gaussian model for the basal area (m2/ha) for young stand monitoring plots.
...
```{r 10-spherical, warning=FALSE, message=FALSE}
## summarize the basal area at plot level
data1<- data%>% 
  group_by(utm_easting,utm_northing) %>%
  summarise(Basal=sum(baha_L))
...
(Figure \@ref(fig:10-spherical)) A semivariogram using the Spherical model for the basal area (m2/ha) for for young stand monitoring plots.
...
```{r 10-exponential,  warning=FALSE, message=FALSE}
## summarize the basal area at plot level
data1<- data%>% 
  group_by(utm_easting,utm_northing) %>%
  summarise(Basal=sum(baha_L))
...
(Figure \@ref(fig:10-exponential)) A semivariogram using the exponential model for the basal area (m2/ha) for for young stand monitoring plots.
...
```{r 10-circular,  warning=FALSE, message=FALSE}
## summarize the basal area at plot level
data1<- data%>% 
  group_by(utm_easting,utm_northing) %>%
  summarise(Basal=sum(baha_L))
...
(Figure \@ref(fig:10-circular)) A semivariogram using the circular model for the basal area (m2/ha) for for young stand monitoring plots.
...
(Figure \@ref(fig:10-circular))Summary of various component of variogram for four different models.
...
![(Figure \@ref(fig:10-circular-2)). Location of Fort Saint Johns TSA and the young stand change monitoring plots.](images/10-FSJ-plot.png){.center}
...
```{r 10-nearest-neighbor-2, warning=FALSE, message=FALSE}
v <- voronoi(dta)
plot(v)
...
(Figure \@ref(fig:10-nearest-neighbor-2)) An intermediate step in creating polygon and rasterizing it over the entire Fort Saint Johns TSA.

**Step 3: Nearest neighbor with five unsampled points to be interpolated at a time and plot the results**

```{r 10-nearest-neighbor-3, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-nearest-neighbor-3)) Predicted basal area over the entire Fort Saint Johns TSA using five nearest neighbor.

**Step 4: Leaflet map for some interactions**

```{r 10-nearest-neighbor-4, echo=FALSE, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-nearest-neighbor-4)) Predicted basal area over the entire Fort Saint Johns TSA using five nearest neighbor projected over the province of British Columbia. 
...
```{r 10-nearest-neighbor-8, warning=FALSE, message=FALSE}
# Map the data
#using package "tmap"
tm_shape(th.clp) +
  ...
  tm_legend(legend.outside=FALSE)
```
(Figure \@ref(fig:10-nearest-neighbor-8)) Predicted basal area over the entire Fort Saint Johns TSA using thiessian polygon.

#### Inverse Distance Weighting

***Inverse distance weighting (IWD)** [@Shepard1968] estimates the variable of interest by assigning more weight to closer points using the weighting function (w) based on the weighting exponent know as power (p) [@Babak2009]. The influence of one data point on the other decreases as the distance increases. Hence, higher power of the exponent will result in point of interest having less effect on the points far from it [@Babak2009]. It is a simple technique that does not require prior information to be applied to spatial prediction [@Shepard1968]. Lower value of exponents mean more averaging, and the weights are more evenly distributed among the surrounding data points [@Shepard1968].
...
```{r 10-idw-4, warning=FALSE, message=FALSE}
tm_shape(r.m) + 
  ...
  tm_legend(legend.outside=FALSE)
```
(Figure \@ref(fig:10-idw-4)) Predicted basal area over the entire Fort Saint Johns TSA using inverse distance weighting.

**Step 5: Leaflet map for some interaction**

```{r 10-idw-5, echo=FALSE, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-idw-5)) Predicted basal area over the entire Fort Saint Johns TSA using inverse distance weighting projected over the province of Brithish Columbia.
...
```{r 10-simple-kriging-4, warning=FALSE, message=FALSE}
...
(Figure \@ref(fig:10-simple-kriging-4)) Variogram models fitted for basal area using the YSM plot data. 

**Step 5: Put all the variogram parameters in a table and see which fits best**

(Figure \@ref(fig:10-simple-kriging-4)) Summary of various components of variogram fro four variogram models.

```{r 10-simple-kriging-5, echo=FALSE, warning=FALSE, message=FALSE, tab.cap = FALSE}
...
```{r 10-simple-kriging-7, warning=FALSE, message=FALSE}
...
```
(Figure \@ref(fig:10-simple-kriging-7)) Predicted basal area using simple kriging.
...
```{r 10-ordinary-kriging-2, warning=FALSE, message=FALSE}
#convert the kriging results to raster
...
```
(Figure \@ref(fig:10-ordinary-kriging-2)) Predicted basal area using ordinary kriging.
...
```{r 10-universal-kriging-2, warning=FALSE, message=FALSE}
#convert the kriging results to raster
...
(Figure \@ref(fig:10-universal-kriging-2)) Predicted basal area using universal kriging.
...
(Figure \@ref(fig:10-universal-kriging-3)) Cross-validation results for different type of kriging.
