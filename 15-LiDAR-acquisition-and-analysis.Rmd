...
(ref:15-high-low-point-density-caption) A cross section of a high density point cloud (A, 80 points·m⁻²), and a lower density point cloud (B, 1 point·m⁻²).(Du Toit, CC-BY-4.0).

```{r 15-high-low-point-density, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:15-high-low-point-density-caption)"
...
(ref:15-LiDAR-Unit-caption) An example of a LiDAR Unit. ('LiDAR machine', @the_center_for_international_forestry_research_cifor_lidar_2014, CC-BY-NC-ND-2.0).
...
(ref:15-LiDAR-Platforms-caption) LiDAR Platforms 1: A terrestrial laser scanner (TLS). ('Lidar P1270901.png', @monniaux_lidar_2007, CC-BY-SA-3.0). 2: A hand-held moblie laser scanner (MLS). (Du Toit, CC-BY-4.0). 3: Car mounted MLS. ('3D mobile mapping unit', @oregon_department_of_transportation_3d_2016, CC-BY-2.0)'. 4: GEDI, a spaceborne LiDAR sensor.('A New Hope: GEDI to Yield 3D Forest Carbon Map', @neon_education_key_2014, CC-BY-2.0).
...
(ref:15-LiDAR-Discrete-Full-Waveform-caption) Difference in the response between discrete echo and full waveform LiDAR. 'Airborne Laser Scanning Discrete Echo and Full Waveform signal comparison', @beck_airborne_2012, CC-BY-SA-3.0).
...
```{r 15-lidar-visualization-static, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Online tool for visualizing and exploring the properties of a small LiDAR point cloud. Data from City of Vancouver and licensed under the <a href='",covlicence,"'>Open Government License - Vancouver</a>. <a href='https://www.opengeomatics.ca/viz/lidar-viz/'>Click here to access the interactive tool.</a> Juno Yoon, CC-BY-SA-4.0.")
...
(ref:15-las-dem-caption) A digital elevation model (DEM) produced from LiDAR.(Du Toit, CC-BY-4.0).

```{r 15-las-dem, fig.cap = fig_cap, out.width="70%", echo = FALSE}
fig_cap = "(ref:15-las-dem-caption)"
...
(ref:15-las-CHM-2D-caption) A canopy height model (CHM), also known as a digital surface model (DSM) produced from LiDAR. (Du Toit, CC-BY-4.0).

```{r 15-las-CHM-2D, fig.cap = fig_cap, out.width="70%", echo = FALSE}
fig_cap = "(ref:15-las-CHM-2D-caption)"
...
(ref:15-las-hmax-2D-caption) Maximum heights of 10 meter cells using an area-based approach. (Du Toit, CC-BY-4.0).

```{r 15-las-hmax-2D, fig.cap = fig_cap, out.width="70%", echo = FALSE}
fig_cap = "(ref:15-las-hmax-2D-caption)"
...
(ref:15-Processing-Flowchart-caption) A typical LiDAR processing workflow. After acquiring data, pre-processing is necessary to clean and normalize the point cloud. After this a CHM can be created to detect tree tops, before segmentating the point cloud based on these points. (Du Toit, CC-BY-4.0).

```{r 15-Processing-Flowchart, fig.cap = fig_cap, out.width="90%", echo = FALSE}
fig_cap = "(ref:15-Processing-Flowchart-caption)"
...
(ref:15-las-treetops-caption) Example of tree tops detected using the `find_trees` function. (Du Toit, CC-BY-4.0).

```{r 15-las-treetops, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:15-las-treetops-caption)"
...
(ref:15-multipath-error-caption) Example of multipath errors that can occur. The green dot is where our point is actually located in space, while the blue point shows where the point is placed in the point cloud. The red line shows the path of the reflected pulse. (Du Toit, CC-BY-4.0).

```{r 15-multipath-error, fig.cap = fig_cap, out.width="70%", echo = FALSE}
fig_cap = "(ref:15-multipath-error-caption)"
knitr::include_graphics("images/15-Multipath-Error.png")
```

## Software and Analysis Tools

As LiDAR acquisition becomes cheaper, more tools are becoming available to do analysis. In this chapter, we use the a free and open source [R package liR](https://cran.r-project.org/web/packages/lidR/); which can be used for the entire process of analyzing a point cloud. Several other options exist, such as [A Shiny-based Application for Extracting Forest Information from LiDAR data](https://github.com/carlos-alberto-silva/weblidar-treetop) (also in R), the [Digital-Forestry-Toolbox](http://mparkan.github.io/Digital-Forestry-Toolbox/) which is available for MATLAB, the [software FUSION](http://forsys.cfr.washington.edu/fusion/fusion_overview.html), which has been developed by the USDA Forest Service, and finally the [free and open source GIS software QGIS](https://www.qgis.org/en/site/). Paid software is also frequently used in LiDAR processing (examples include [Esri's ArcGIS Pro](https://www.esri.com/en-us/home), and [LAStools](https://rapidlasso.com/lastools/)), although the cost can be prohibitive. Finally, the [open source software CloudCompare](https://www.danielgm.net/cc/) can be incredibly useful for both viewing and manually clipping/editing point clouds.

## Case Study: Creating LiDAR Metrics from a Raw Point Cloud

For this case study, we will be using a clipped .las file from the [2018 open LiDAR dataset](https://opendata.vancouver.ca/explore/dataset/lidar-2018/information/) of the City of Vancouver and UBC Endowment Lands in British Columbia (we randomly selected a 200 x 200 m portion of the '4840E_54550N' tile using CloudCompare), and the `lidR` package in R [@city_of_vancouver_lidar_nodate],[@roussel_lidr_2020]. The script to process this data is included in the data folder of the GitHub repository, and you can use the [online `lidR` book](https://r-lidar.github.io/lidRbook/) to get a more in depth understanding of the functions we apply below [@roussel_lidr_2021].
...
(ref:15-las-normalize-caption) A normalized point cloud. (Du Toit, CC-BY-4.0).

```{r 15-las-normalize, fig.cap = fig_cap, out.width="60%", echo = FALSE}
fig_cap = "(ref:15-las-normalize-caption)"
...
(ref:15-las-segmented-caption) A segmented point cloud using the algorithm based on @dalponte_tree-centric_2016. (Du Toit, CC-BY-4.0).
...
In Figure \@ref(fig:15-LiDAR-derivatives-map) below, explore some of the LiDAR derivatives that we produced in the case study above. We can see how the pattern for maximum height follows the mean height quite closely. In addition, we can see that the 15th percentile of heights (ZQ 15) also broadly follows this pattern. However, we can see that the standard deviation of heights in the tall areas are quite variable, and that very little ground is visible in our tile (as the percentage of points above 2 m is very high almost everywhere).
...
(ref:15-LiDAR-derivatives-map-caption1) Explore different area-based metrics produced in the case study above. Hover over tile icon (top left) to turn different layers on or off. Du Toit, CC-BY-4. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/LiDAR-acquisition-and-analysis.html#fig:5-LiDAR-derivatives-map.

(ref:15-LiDAR-derivatives-map-caption2) Explore different area-based metrics produced in the case study above. Hover over tile icon (top left) to turn different layers on or off. Du Toit, CC-BY-4.

```{r 15-LiDAR-derivatives-map, echo=FALSE, fig.cap = fig_cap}
if (knitr:::is_latex_output()) { fig_cap = "(ref:15-LiDAR-derivatives-map-caption1)"
  knitr::include_graphics("images/15-LiDAR-derivatives-map.png")
} else { fig_cap = "(ref:15-LiDAR-derivatives-map-caption2)"
