...
fig_cap <- paste0("Concept of LiDAR. A light signal is emitted by the scanner and reflected off the target.('Concept of LiDAR', @Cartographer3d2021, CC-BY-SA-4.0).")
...
fig_cap <- paste0("A cross section of a high density point cloud (A, 80 points·m^-2^), and a lower density point cloud (B, 1 point·m^-2^).(Du Toit, CC-BY-4.0).")
...
fig_cap <- paste0("Two examples of how beam divergence affects footprint size. (Du Toit, CC-BY-4.0).")
...
fig_cap <- paste0("Scan angle is the angle from nadir at which the laser is pointing. The scan angle and aircraft flight height together are responsible for swath width. (Du Toit, CC-BY-4.0).")
...
fig_cap <- paste0("An example of a LiDAR Unit. ('LiDAR machine', @TheCenterforInternationalForestryResearchCIFOR2014, CC-BY-NC-ND-2.0).")
...
fig_cap <- paste0("An on board GNSS is required toprovide an accurate location of the LiDAR scanner in space. ('A key component of a lidar system is a GPS', @NEONEducation2014,CC-BY-NC-SA-2.0).")
...
fig_cap <- paste0("Drone mounted LiDAR. This technology is rapidly developing, with many companies working towards creating units capable of acquiring high density datasets over large areas. ('LiDARUSA Snoopy 120 LiDAR', @Clapurhands2019,  CC-BY-SA-4.0).")
...
fig_cap <- paste0("LiDAR Platforms 1: A terrestrial laser scanner (TLS). ('Lidar P1270901.png', @Monniaux, CC-BY-SA-3.0). 2: A hand-held moblie laser scanner (MLS). (Du Toit, CC-BY-4.0). 3: Car mounted MLS. ('3D mobile mapping unit', @OregonDepartmentofTransportation2016, CC-BY-2.0)'. 4: GEDI, a spaceborne LiDAR sensor.('A New Hope: GEDI to Yield 3D Forest Carbon Map', @NEONEducation2014, CC-BY-2.0).")
...
fig_cap <- paste0("Differents in the response between discrete echo and full waveform LiDAR. 'Airborne Laser Scanning Discrete Echo and Full Waveform signal comparison', @Beck2012, CC-BY-SA-3.0).")
...
fig_cap <- paste0("A digital elevation model (DEM) produced from LiDAR.(Du Toit, CC-BY-4.0).")
...
fig_cap <- paste0("A canopy height model (CHM), also known as a digital surface model (DSM) produced from LiDAR. (Du Toit, CC-BY-4.0).")
...
fig_cap <- paste0("Maximum heights of 10 meter cells using an area-based approach. (Du Toit, CC-BY-4.0).")
...
fig_cap <- paste0("A typical LiDAR processing workflow. After acquiring data, pre-processing is necessary to clean and normalize the point cloud. After this a CHM can be created to detect tree tops, before segmentating the point cloud based on these points. (Du Toit, CC-BY-4.0)." )
...
fig_cap <- paste0("Example of tree tops detected using the `find_trees` function. (Du Toit, CC-BY-4.0)." )
...
fig_cap <- paste0("Example of multipath errors that can occur. The green dot is where our point is actually located in space, while the blue point shows where the point is placed in the point cloud. The red line shows the path of the reflected pulse. (Du Toit, CC-BY-4.0)." )
...
fig_cap <- paste0("A normalized point cloud. (Du Toit, CC-BY-4.0)." )
...
fig_cap <- paste0("A segmented point cloud using the algorithm based on @Dalponte2016. (Du Toit, CC-BY-4.0).")
...
fig_cap <- paste0("Explore different area-based metrics produced in the case study above. Hover over tile icon (top left) to turn different layers on or off. <a href='",baseurl,"#fig:15-map'>Animated figure can be viewed in the web browser version of the textbook</a>.Data from City of Vancouver and licensed under <a href='",covlicence,"'>Open Government License - Vancouver</a>.Du Toit, CC-BY-4.").
