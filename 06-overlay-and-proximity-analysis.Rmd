...
## Overlay Methods
In the above example, the data was provided already compiled. However, we often have to gather the data ourselves. Overlay methods allow us to get more information about a spatial area, based on other spatial attributes. Say, for example, you are assessing the ecosystem services of your city's trees. This can be done at several scales: are you concerned with the city as a whole? Neighborhoods? Census tracts? Or are you assessing trees individually? The answer to this is dependent on the question. For the following example, we will be assessing the ecosystem services of trees by census tract within an Ottawa neighborhood (ward??).
...
### Clip
Ottawa has a lot of data, we want something easier to work with. So we are looking at Barrhaven, the most populous Ward in Ottawa. When we download data from Ottawa, we will clip it to the size of the neighborhood. (show clip example). A clip is effectively a cookie-cutter, where the boundaries of the clipping feature become the boundaries of the clipped feature. (the clipped extent is at least as small at the clipping extent)
