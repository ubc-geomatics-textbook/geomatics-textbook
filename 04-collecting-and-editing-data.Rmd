...
var part2 = "T00:00&format=png&maxlevel=40&type=1&lang=en"
...
preload(
  ...
  part1 + tec1date.toISOString().split('T')[0] + part2,
  part1 + tec2date.toISOString().split('T')[0] + part2,
  part1 + tec3date.toISOString().split('T')[0] + part2,
  part1 + tec4date.toISOString().split('T')[0] + part2,
  part1 + tec5date.toISOString().split('T')[0] + part2,
  part1 + tec6date.toISOString().split('T')[0] + part2,
  part1 + tec7date.toISOString().split('T')[0] + part2
)
```
...
Practically speaking, all of the above assumes that there are absolutely no errors in this system, but there are in fact many factors that impact the timing of the signal, which are discussed in the next section. One important factor is clock error. The clocks on the satellites are highly precise atomic clocks that are synchronized and periodically corrected. This clock synchronization among the satellites is important because it allows us, on the ground with our receivers, to rely on a consistent and comparable time stamp in the GNSS signal. So when we get a signal from one satellite and calculate the time difference, that time difference should be comparable to the time difference we get with any other satellite.

However, the clocks in our receiver have no where near the same precision and are not synchronized with the clocks on the satellites. This means that the current time in our receiver on the ground is not the same current time as the satellite orbiting overhead. No matter how small this time difference is between the satellite and receiver, it is multiplied by the speed of light and ultimately impacts our estimation of position. For example, if the current time on the satellite differed by 0.001 s or 1 ms from our receiver, then our range to a single satellite will be off by about 300 km! For this reason, we need a fourth satellite to solve for the final position. This process of intersecting spheres from three or more satellites is known as **trilateration**.
...
```{r, echo=FALSE}
doy <- yday(Sys.Date())
tec1date <- as.Date(doy-2, origin = paste0(format(Sys.Date(), "%Y"),"-01-01"))
tec2date <- as.Date(doy-3, origin = paste0(format(Sys.Date(), "%Y"),"-01-01"))
tec3date <- as.Date(doy-4, origin = paste0(format(Sys.Date(), "%Y"),"-01-01"))
tec4date <- as.Date(doy-5, origin = paste0(format(Sys.Date(), "%Y"),"-01-01"))
tec5date <- as.Date(doy-6, origin = paste0(format(Sys.Date(), "%Y"),"-01-01"))
tec6date <- as.Date(doy-7, origin = paste0(format(Sys.Date(), "%Y"),"-01-01"))
tec7date <- as.Date(doy-8, origin = paste0(format(Sys.Date(), "%Y"),"-01-01"))

tec0 <- "https://www.spaceweather.gc.ca/auto_generated_products/tec/latest/tec-en.png"
tec1 <- paste0("https://www.spaceweather.gc.ca/service/tools/tec/?date=",tec1date,"T00:00&format=png&maxlevel=40&type=1&lang=en")
tec2 <- paste0("https://www.spaceweather.gc.ca/service/tools/tec/?date=",tec2date,"T00:00&format=png&maxlevel=40&type=1&lang=en")
tec3 <- paste0("https://www.spaceweather.gc.ca/service/tools/tec/?date=",tec3date,"T00:00&format=png&maxlevel=40&type=1&lang=en")
tec4 <- paste0("https://www.spaceweather.gc.ca/service/tools/tec/?date=",tec4date,"T00:00&format=png&maxlevel=40&type=1&lang=en")
tec5 <- paste0("https://www.spaceweather.gc.ca/service/tools/tec/?date=",tec5date,"T00:00&format=png&maxlevel=40&type=1&lang=en")
tec6 <- paste0("https://www.spaceweather.gc.ca/service/tools/tec/?date=",tec6date,"T00:00&format=png&maxlevel=40&type=1&lang=en")
tec7 <- paste0("https://www.spaceweather.gc.ca/service/tools/tec/?date=",tec7date,"T00:00&format=png&maxlevel=40&type=1&lang=en")
```
