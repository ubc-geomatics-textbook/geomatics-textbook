...
3.  Understand the basic processing steps required to use LiDAR data in a forestry and ecology context
:::
...
Typical LiDAR systems use the time of flight method to produce 3D data. A laser ranging instrument produces a short, intense pulse of light from the instrument to a target being measured. Some of this energy is then reflected back to the instrument, where it is recorded (as seen in Figure 15.1). Since the speed of light, and the location of the laser ranging instrument is known, we can calculate the position of the target by timing how long it takes between the the pulse being emitted and received. If we shoot many pulses of light towards a target, we can create a 3D point cloud of our target (see Figure 15.8 as an example of a forest scene).

![Figure 15.1 Concept of LiDAR.](images/15-Concept_of_LiDAR.png){.center}

When thinking about a LiDAR dataset, there are several parameters to take note of. It is important to know what the technical specifications of the data collection parameters were (for example, what was is the point density?), and how the data was collected (what plaform was used). A LiDAR dataset includes several pieces of information for each 3D point. X,Y,Z location data is included, as well as information about how the data was collected. In many cases, this includes things like return number, scan angle, gps time, classification, and intensity. We will discuss these concepts in more detail below.
...
![Figure 15.2 LiDAR System.](images/15-LiDAR_System.png){width="50%"}

## LiDAR History and Use

Early LiDAR point densities were low (1 - 5 p/m^2^), which limited researchers to area based measurements of forest volume and biomass in the 1980s (CITE NELSON 2013). Technological improvements have meant that individual tree crown level analysis is becoming possible (CITE Jakubowski 2013).

-   What kind of technological improvements - specifically pulse rates, storage? [see proposal]

    -   White et al - LiDAR citation, Open source textbook for ITD?

-   what can we do with LiDAR, what are people currently doing with the technology?
...
-   Include HIGH POINT DENSITY vs LOW POINT DENSITY
...
-   return number, gps time, classification, and intensity!

![Figure 15.3 LiDAR Unit.](images/15-LiDAR_Unit.jpg){width="50%"}
...
![Figure 15.4 XYZ Coordinates.](images/15-XYZ_coordinates.jpg){width="75%"}
...
![Figure 15.5 Drone mounted LiDAR.](images/15-LiDAR_on_Drone_2.jpg){width="75%"}
...
![Figure 15.6 LiDAR Platforms 1:SOURCE, 2:SOURCE, 3:SOURCE, 4:SOURCE.](images/15-LiDAR_Platforms.png){width="75%"}
...
Issue of data storage - now becoming a viable alternative. Look at Figure 15.7 for example
...
![Figure 15.7 LiDAR Discrete vs Full Waveform.](images/15-LiDAR_Discrete_Full_Waveform.png){width="60%"}
...
![Figure 15.8 typical LiDAR point cloud.](images/15-las_denoise.png){width="90%"}
...
-   Include short blurb about the two funamental types of approach here? \#\#\# Grid Metrics

-   What are these used for

![Figure 15.11 Maximum cell heights in an area-based approach.](images/15-las_hmax_2D.png){.center}

![Figure 15.12 Example of all the metrics you can produce... replace with leaflet map in 'your turn' section](images/15-las_metrics_2D.png){.center}
