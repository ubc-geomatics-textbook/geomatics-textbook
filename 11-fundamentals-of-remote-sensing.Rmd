...
```{r 11-nadar, fig.cap = fig_cap, out.width="100%", echo = FALSE}
...
```{r 11-hermosilla-canada-landcover, fig.cap = fig_cap, echo = FALSE, out.width="100%"}
...
```{r 11-EMS, fig.cap = fig_cap, out.width="100%", echo = FALSE}
...
wish a visual tour of the EMS you can explore ["Tour of the electromagnetic spectrum"
online](https://books.google.ca/books?id=DfLPpxogdM4C&pg=PP1#v=onepage&q&f=false)
...
```{r 11-scattering, fig.cap = fig_cap, echo = FALSE, out.width="100%"}
...
```{r 11-solar-radiation, fig.cap = fig_cap, echo = FALSE, out.width="100%"}
...
```{r 11-reflectors, fig.cap = fig_cap, out.width="100%", echo = FALSE}
...
```{r 11-ems-visualization-static, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste0("Online tool for visualizing the spectral signatures for different land covers using hyperspectral Hyperion satellite imagery. Data from United States Geological Survey - Licensed by U.S. Public Domain. <a href='https://www.opengeomatics.ca/viz/emspectrum-viz/'>Click here to access the interactive tool.</a> , CC-BY-SA-4.0.")
...
data over space and time in [Chapter 12](./remote-sensing-systems.html) and [Chapter 15](./LiDAR-acquisition-and-analysis.html).
...
detail in [Chapter 12](./remote-sensing-systems.html), so for now we'll focus on how spectral information
can be useful.
...
(ref:11-specsig-caption) Five spectral signatures of various living and non-living samples collected using an ASD FieldSpec3 Imaging Spectroradiometer. Live = live broadleaf, Dead = dead broadleaf, Fir = Douglas-fir needles on branch, Rock = rock, soil = dry humus soil.

```{r 11-specsig, fig.cap = fig_cap, out.width="100%", echo = FALSE}
    fig_cap = "(ref:11-specsig-caption)"
...
(ref:11-radiometric-caption) Discernable values of sensors with 1, 2 and 4 bot radiometric resolutions. Created by Paul Hacker CC BY-SA 4.0.

```{r 11-radiometric, fig.cap = fig_cap, echo = FALSE, out.width="100%"}
    fig_cap = "(ref:11-radiometric-caption)"
...
observations at night. [Chapter 12](./remote-sensing-systems.html) will discuss radar in more detail.
...
LIDAR will be presented in [Chapter 15](./LiDAR-acquistion-and-anlysis.html).
...
```{r 11-hermo-false, fig.cap = fig_cap, out.width="100%", echo = FALSE}
...
```{r 11-hermo-disturb, fig.cap = fig_cap, out.width="100%", echo = FALSE}
