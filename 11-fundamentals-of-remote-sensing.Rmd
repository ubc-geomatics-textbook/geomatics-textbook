...
```{r 11-nadar, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
    fig_cap <- paste("Images of the Arc de Triomphe in Paris, France taken by Tournachone from a balloon in 1968. Animated figure can be viewed in the web browser version of the textbook.")
...
```{r 11-hermosilla-canada-landcover, fig.cap = fig_cap, echo = FALSE}
    fig_cap <- paste("Landcover classification of Canada generated by Hermosilla et al., 2018 for the year 2005. Animated figure can be viewed in the web browser version of the textbook.")
...
```{r 11-EMS, fig.cap = fig_cap, out.width = "200%", echo = FALSE}
    fig_cap <- paste("Electromagnetic (also known as Milton) spectrum depicting the type, wavelength, frequency and black body emission temperature. Animated figure can be viewed in the web browser version of the textbook. Credit: Inductiveload, NASA.")
...
```{r 11-solar-radiation, fig.cap = fig_cap, echo = FALSE}
    fig_cap <- paste("Solar radiation spectrum from 250 - 2500 nm. Irradiance measurements at the top of the atmosphere (yellow) and sea level (red) are depicted. The grey line represents the theoretical curve of a 5250 degree C blackbody spectrum. Animated figure can be viewed in the web browser version of the textbook. Created by Robert A. Rohde for Global
...
```{r 11-reflectors, fig.cap = fig_cap, out.width="75%",out.height="75%", echo = FALSE}
    fig_cap <- paste("Depictions of specular and diffuse reflectors. Animated figure can be viewed in the web browser version of the textbook. Created by Ant Beck (CC BY-SA 3.0), 2012.")
...
```{r 11-specsig, fig.cap = fig_cap, out.width = "200%", echo = FALSE}
    fig_cap <- paste("Five spectral signatures of various living and non-living samples collected using an ASD FieldSpec3 Imaging Spectroradiometer. Animated figure can be viewed in the web browser version of the textbook. Live = live broadleaf, Dead = dead broadleaf, Fir = Douglas-fir needles on branch, Rock = rock, soil = dry humus soil.")
...
```{r 11-radiometric, fig.cap = fig_cap, echo = FALSE}
    fig_cap <- paste("Discernable values of sensors with 1, 2 and 4 bot radiometric resolutions. Animated figure can be viewed in the web browser version of the textbook. Created by Paul Hacker CC BY-SA 4.0.")
...
```{r 11-hermo-false, fig.cap = fig_cap, out.width="90%", echo = FALSE}
    fig_cap <- paste("False colour composite (bands: shortwave infrared, near infrared and red) for 2010. Animated figure can be viewed in the web browser version of the textbook. Each pixel in this image was selected based on its classification as a Best Available Pixel (BAP).")
...
```{r 11-hermo-disturb, fig.cap = fig_cap, out.width="90%", echo = FALSE}
    fig_cap <- paste("Temporal sequences depicting landcover before and after (a) harvesting and (b) fire. The year of change is represented by 'Y'. Animated figure can be viewed in the web browser version of the textbook. Adapted from Hermosilla et al., 2018.")
