...
author <- yml_content[["makingBeautifulMaps"]][["author"]]
coauthor <- yml_content[["makingBeautifulMaps"]][["coauthor"]]
```
...
Maps represent areas larger than we can see, usually from above, and some depict phenomena that cannot be seen [@manson_maps_2017], such as growing seasons. They also illustrate spatial relationships, such as this map showing the percentage of the population aged 65 and over by census tract in Moncton, New Brunswick. We can deduce that those aged 65 years and over are concentrated in certain areas of the city, using census tracts as a proxy.

![Maps illustrate how variables are related in space. [@statistics_canada_thematic_2016] (c) [Statistics Canada](https://www-statcan-gc-ca.eu1.proxy.openathens.net/eng/about/about?MM=as), [Open Government Licence - Canada](https://open.canada.ca/en/open-government-licence-canada).](images/17-moncton.png)
...
There are many different types of maps. Just try doing a Google search, and you will come across descriptions of thematic, cadastral, topographic, and physiographic maps, to name but a few. There is no standard agreement on how many different categories of maps exist. Although some differentiate among as many as five  types of maps based on the functions they serve [@icsm_types_2021], all maps can also be categorized into just two types, **reference** and **thematic**. Reference maps represent the human-made or natural features of the landscape and are sometimes thought of as basemaps. They provide information about a particular location, such as in the example below, depicting the longest rivers in the location of Canada and where they overlap with the United States. Topographic maps based on the [National Topographic System (NTS)](https://www.nrcan.gc.ca/earth-sciences/geography/topographic-information/maps/national-topographic-system-maps/9767) are another example of reference maps.

![One example of a reference map. [@shannon1_longest_2017] (c) [Shannon1](https://en.wikipedia.org/wiki/User:Shannon1), [CC BY-SA](https://creativecommons.org/licenses/by-sa/4.0/).](images/17-longRivers.png)
...
Qualitative thematic maps illustrate the spatial extent of categorical data [@anderson_types_2020], such as the 1956 map bedrock geology map below.

![A thematic map using qualitative categorical data. [@nicholson_bedrock_1956] (c) Comtois and Nicholson, [CC BY-NC-SA 3.0](https://creativecommons.org/licenses/by-nc-sa/3.0/).](images/17-geology.png)
...
![A thematic map depicting population data as a percentage. [@statistics_canada_illustrated_2017] (c) [Statistics Canada](https://www-statcan-gc-ca.eu1.proxy.openathens.net/eng/about/about?MM=as), [Open Government License - Canada](https://open.canada.ca/en/open-government-licence-canada).](images/17-population.png)

## Choropleth Maps

There are many different types of thematic types. A commonly used thematic map is a **choropleth** map, which applies different colours or shading to the entire extent of a predefined aereal unit [@weiss_population_2008]. Data used in choropleth maps must be standardized and represented as rates or ratios rather than raw counts or numbers.
...
(ref:17-dotDensity-caption) Dot density map showing the distribution of Cantonese and Mandarin mother tongue language speakers by census tract [Household Population by Mother Tongue](https://www12.statcan.gc.ca/census-recensement/2016/ref/dict/pop095-eng.cfm) retrieved from [SimplyAnalytics](https://simplyanalytics.com/).

```{r 17-dotDensity, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-dotDensity-caption)"
knitr::include_graphics("images/17-dotDensity.png")
```

## Isoline Maps

(ref:17-isoline-caption) An isoline map representing growing seasons from the _Atlas of Canada_. Isolines are lines which connect points with identical values.  ('Growing Seasons', [@nicholson_growing_1950], CC BY-NC-SA 3.1) retrieved from [David Rumsey Map Collection](https://www.davidrumsey.com/luna/servlet/s/c68r55).

```{r 17-isoline, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-isoline-caption)"
knitr::include_graphics("images/17-isoline.png")
```

<!--  - 17-isoline.png from row 23 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192) -->
...
(ref:17-cartogram-caption) A effective illustration of a cartogram representing the population (%) with no access to clean drinking water. ([@worldmapper_no_2015], CC BY-NC-SA 4.0).

```{r 17-cartogram, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-cartogram-caption)"
knitr::include_graphics("images/17-cartogram.png")
```
<!-- 
- 17-cartogram.png from row 42 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192)

_Note: Paul, see example interactive quiz at bottom of this [Introduction to Geomatics](https://openpress.usask.ca/introgeomatics/chapter/thematic-maps/) OER textbook._
-->
## Additional Resources on Types of Maps

- [Thematic Maps](https://openpress.usask.ca/introgeomatics/chapter/thematic-maps/)
- [Choropleth Maps](https://www.axismaps.com/guide/choropleth)
- [Dot Density Maps](https://www.axismaps.com/guide/dot-density)
- [Dot density map](http://wiki.gis.com/wiki/index.php/Dot_density_map)
- [Types of Maps](https://www.e-education.psu.edu/geog486/node/641)
...
(ref:17-figure-caption) The components of effective map composition. ('Map composition.', @manson_composition_2017, CC BY-NC-SA 4.0)

```{r 17-figure, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-figure-caption)"
knitr::include_graphics("images/17-figure.png")
```
<!-- 
- 17-figure.png from row 32 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192)
...
(ref:17-text-caption) Typography standards for cartography. ('Cartographic style guide.', @gaba_cartographic_2014, CC BY 2.5).

```{r 17-text, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-text-caption)"
knitr::include_graphics("images/17-text.png")
```
<!-- 
- 17-text.png from row 43 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192)
...
(ref:17-scale-caption) Six maps of Ottowa at different scales from the _Atlas of Canada_. ('Comparison of scales', @nicholson_comparison_1958, CC BY-NC-SA 3.1).

```{r 17-scale, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-scale-caption)"
knitr::include_graphics("images/17-scale.png")
```
<!-- 
- 17-scale.png from row 21 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192)
...
(ref:17-symbolization-caption) The components of symbolization. ('Symbolization', @manson_symbolization_2013, CC BY-NC-SA 4.0).

```{r 17-symbolization, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-symbolization-caption)"
knitr::include_graphics("images/17-symbolization.png")
```

## Separable

(ref:17-separable-caption) Separable and integral visual combinations delineated by value/shape and heighth/width, respectively. (@elmer_symbol_2012)

```{r 17-separable, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-separable-caption)"
knitr::include_graphics("images/17-separable.png")
```
<!-- 
- 17-separable.png from row 33, page 7 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192)
...
(ref:17-graduated-caption) Graduated symbols vary symbol size representing a category of values. ('Proportional & Graduated Symbols', @robinson_layout_2020, CC BY-NC-SA 4.1).

```{r 17-graduated, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-graduated-caption)"
knitr::include_graphics("images/17-graduated.png")
```
<!-- 
- 17-graduated.png from row 45 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192)
--> 

## Configurable

(ref:17-configurable-caption) Separable and integral visual combinations delineated by value/shape and heighth/width, respectively. (@elmer_symbol_2012)

```{r 17-configurable, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-configurable-caption)"
knitr::include_graphics("images/17-configurable.png")
```
<!-- 
- 17-configurable.png from row 33 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192)
-->

## Proportional

(ref:17-proportional-caption) Proportional symbols vary size of symbols in relation to their attribute value. ('Proportional & Graduated Symbols', @robinson_layout_2020, CC BY-NC-SA 4.1.)

```{r 17-proportional, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-proportional-caption)"
knitr::include_graphics("images/17-proportional.png")
```

<!-- 
- 17-proportional.png from row 45 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192)
-->

## Line Weight
(ref:17-lineWeight-caption) Varying the thickness of different lines in the map can help focus attention and differentiate between map features. ('Map Elements and Design Principles', @ingram_map_2021, CC-BY-SA-4.0).

```{r 17-lineWeight, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-lineWeight-caption)"
knitr::include_graphics("images/17-lineWeight.png")
```
...
Choice of colours should also take into account that some people are colourblind, especially when choosing a colour scheme. The [online tool Colorbrewer 2.0](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3) helps cartographers evaluate how easy it is to differentiate among colours in a colour scheme.

Colour is actually comprised of three elements: hue, chroma, and lightness, or value.

## Hue

(ref:17-hue-caption) Hues are what we commonly think of as the names of colours and distinguish between qualitative data. ('Hue', @manson_hue_2013, CC BY-NC-SA 3.0).

```{r 17-hue, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-hue-caption)"
knitr::include_graphics("images/17-hue.png")
```

## Chroma

(ref:17-chroma-caption) Higher values represent lighter colorus while lower values refer to dark colours. ('Chroma', @anderson_chroma_2020, CC BY-NC-SA 4.0).

```{r 17-chroma, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-chroma-caption)"
knitr::include_graphics("images/17-chroma.png")
```

## Lightness

(ref:17-lightness-caption) Higher values represent lighter colours while lower values refer to darker colours. ('Value', @manson_value_2013, CC BY-NC-SA 4.0).

```{r 17-lightness, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-lightness-caption)"
knitr::include_graphics("images/17-lightness.png")
```

## Bivariate Colour Schemes

(ref:17-bivariate-caption) Different examples of bivariate legends. ('Bivariate', @axis_maps_bivariate_2020, CC BY-NC-SA 4.0).

```{r 17-bivariate, fig.cap = fig_cap, out.width="80%", echo = FALSE}
fig_cap = "(ref:17-bivariate-caption)"
knitr::include_graphics("images/17-bivariate.png")
```
<!-- 
## Colour Pickers
...
Good map design effectively integrates map elements with design principles and symbolization to produce a persuasive map [@deluca_design_2017].
<!-- 
## Subject
...
- hierarchy.png from row 44 of [OER Content Sharing](https://docs.google.com/spreadsheets/d/1LqzXn00wMeIjHWstNT3tMImNDZirLGc3g72jFOQc_8I/edit#gid=817407192) for labeling hierarchy
