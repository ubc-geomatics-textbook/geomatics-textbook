...
**Geostatistics** uses the metrics based on statistical tools that are used to characterize the distribution of an event across the geographical region of interest [@Getis2004].In context of spatial data, it is important to understand what is occurring where. Sometimes there is a detailed and precise information of an event across the geographical area such that a continuous map can be built out of that information. While, most of the time , only discrete and unrepresentative information on the spatial event is collected, which doesnot allow to create a precise continuous surface of the event over entire geographic area of interest. In such scenario, we should focus on two things; 
...
2. On the other hand, a spatial data can carry the unsampled within a study area. 
...
In cluster sampling, rather than sampling individual units, which might be geographically spread over great distances, we can sample groups (clusters) that occur naturally in the population [@Teddlie2007]. Cluster sampling is employed when we want to be more efficient in terms of the use of time and money to generate a more efficient probability sample [@Teddlie2007].
...
This type of sampling is used when we want to select samples that will represent broader groups as closely as possible [@Teddlie2007]. One of the example of representative sampling is Census survey in Canada.
...
proj4string(grd) <- proj4string(dta)
```
**Step 2: Calculate the mean of the basal area so that we can use it in our simple kriging, which assumes that mean is a known constant over entire domain**
...
**Step 3: Start by fitting the semivariogram model for the basal area and see which model best fit the data**
...
**Step 4: Put all the variogram parameters in a table and see which fits best**
...
##### Step 6: Visualize the predicted surface using "spplot" {-}
...
```
**Step 6: Visualize the predicted surface using "tmap"**
...
**Step 2: Visualize the results as we ussually do using tmap**
...
Spatial lag models assume that the spatial autocorrelation only exist in the response variable or the variable of interest [@Anselin1998]. Spatial lag model has the follwoing general mathematical formulations:
...
Spatial error models assume that the spatial autocorrelation exists in the residuals or the error term of the regression equation [@Anselin1998]. The general mathematical formula for saptial error model is given below: 
...
1. Explain probability and no-probability sampling.
2. Define spatial autocorrelation and semivariogram.
3. When do you use spatial contiguity and nearest neighbor?
