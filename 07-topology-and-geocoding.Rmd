...
author <- yml_content[["topologyAndGeocoding"]][["author"]]
coauthor <- yml_content[["topologyAndGeocoding"]][["coauthor"]]
```
...
(ref:07-native-land-leaflet-caption1) Non-planar topology of 36 indigenous territories overlapping Vancouver Island, British Columbia. Data from @native_land_native_nodate, CC0. Animated figure can be viewed in the web browser version of the textbook: https://opengeomatics.ca/topology-and-geocoding.html.

(ref:07-native-land-leaflet-caption2) Non-planar topology of 36 indigenous territories overlapping Vancouver Island, British Columbia. Data from @native_land_native_nodate, CC0.

```{r 07-native-land-leaflet, fig.cap = fig_cap, out.width= "75%", echo = FALSE}
if (knitr:::is_latex_output()) { fig_cap ="(ref:07-native-land-leaflet-caption1)"
knitr::include_graphics("images/07-native-land-leaflet.png")
} else { fig_cap = "(ref:07-native-land-leaflet-caption2)"
m
}
```
...
#countries <- st_read("Data/02/world-countries.geojson")
countries <- rgdal::readOGR("Data/02/world-countries.geojson", verbose = F)
...
(ref:7-haida-gwaii-leaflet-caption1) Singlepart geometry of the Haida Gwaii archipelago off the west coast of British Columbia, Canada. Hover over the islands to see the names. Polygon data from @statistics_canada_statistics_nodate and island placenames from @natural_resources_canada_natural_nodate. Open Government License - Canada. Animated figure can be viewed in the web browser version of the textbook: https://ubc-geomatics-textbook.github.io/geomatics-textbook/topology-and-geocoding.html

(ref:7-haida-gwaii-leaflet-caption2) Singlepart geometry of the Haida Gwaii archipelago off the west coast of British Columbia, Canada. Hover over the islands to see the names. Polygon data from @statistics_canada_statistics_nodate and island placenames from @natural_resources_canada_natural_nodate. Open Government License - Canada.

```{r 7-haida-gwaii-leaflet, fig.cap = fig_cap, out.width= "50%", echo = FALSE}
if (knitr:::is_latex_output()) { fig_cap = "(ref:7-haida-gwaii-leaflet-caption1)"
  knitr::include_graphics("images/07-haida-gwaii-leaflet.png")
} else { fig_cap ="(ref:7-haida-gwaii-leaflet-caption2)"
m2
}
```
...
Topologically, holes in polygons imply that another polygon shares an adjacent boundary where the hole exists, for example, from the union of two layers (see [Chapter 6](./overlay-and-proximity-analysis.html)). In our example, the lake would comprise its own polygon that would completely fill the hole.
...
(ref:7-2d-alpha-hull-caption1) Concave alpha hull generates an alpha shape around a set of points. Online version of the figure is animated by alpha values from 0 to 1 by increments of 0.05. Pickell, CC-BY-SA-4.0.  Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology-and-geocoding.html#fig:7-2d-alpha-hull.

(ref:7-2d-alpha-hull-caption2) Concave alpha hull generates an alpha shape around a set of points. Online version of the figure is animated by alpha values from 0 to 1 by increments of 0.05. Pickell, CC-BY-SA-4.0.


``` {r 7-2d-alpha-hull, echo=FALSE, out.width = '75%', fig.cap=fig_cap}
if (knitr:::is_latex_output()) { fig_cap = "(ref:7-2d-alpha-hull-caption1)"
  knitr::include_graphics("images/07-2d-alpha-hull.png")
} else { fig_cap = "(ref:7-2d-alpha-hull-caption2)"
...
In this next section, a number of topologies are described that are important for 3D modeling and several examples are given using LiDAR (Light Detection and Ranging), which is the topic of [Chapter 15](./LiDAR-acquisition-and-analysis.html). It is beyond the scope of this chapter to discuss the technology of LiDAR, so the reader is referred to [Chapter 15](./LiDAR-acquisition-and-analysis.html) for a more in-depth discussion of LiDAR.
...
(ref:7-platonic-polyhedrons-caption1) The five Platonic solids are examples of regular, convex polyhedrons and multipatch geometries. From left to right: tetrahedron (4 faces); hexahedron (6 faces); octahedron (8 faces); dodecahedron (12 faces); and icosahedron (20 faces). @cyp_polyhedra_2005, CC-BY-SA-3.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology-and-geocoding.html#fig:7-platonic-polyhedrons

(ref:7-platonic-polyhedrons-caption2) The five Platonic solids are examples of regular, convex polyhedrons and multipatch geometries. From left to right: tetrahedron (4 faces); hexahedron (6 faces); octahedron (8 faces); dodecahedron (12 faces); and icosahedron (20 faces). @cyp_polyhedra_2005, CC-BY-SA-3.0.

``` {r 7-platonic-polyhedrons, echo=FALSE, out.width = '75%', fig.cap=fig_cap}
if (knitr:::is_latex_output()) { fig_cap = "(ref:7-platonic-polyhedrons-caption1)"
  knitr::include_graphics("images/07-platonic-polyhedrons.png")
} else { fig_cap = "(ref:7-platonic-polyhedrons-caption2)"
...
```{r setup-3d-alpha-hull-deciduous, echo=FALSE, cached=T, warning=F, include=F}
las <- readLAS("Data/07/07-deciduous-tree.las")
z <- las[,3]
x <- las[,1]
y <- las[,2]
...
(ref:7-3d-alpha-hull-deciduous-caption2) 3D concave alpha hull for a deciduous tree (species here). The alpha hull was generated using alpha = 0.05. Data collected by Spencer Dakin Kuiper with a GeoSlam terrestrial laser scanner in Vancouver, Canada. Pickell, CC-BY-SA-4.0.
...
knitr::include_graphics("images/07-3d-alpha-hull-deciduous.png")
...
``` {r setup-3d-alpha-hull-conifer, echo=F, cached=T, warning=F, include=F}
...
(ref:7-3d-alpha-hull-conifer-caption1) 3D convex alpha hull for a coniferous tree (Abies lasiocarpa). The alpha hull was generated using alpha = 0.1. Data collected by Yangqian (Frederick) Qi with a terrestrial laser scanner in British Columbia, Canada. Pickell, CC-BY-SA-4.0. Animated figure can be viewed in the web browser version of the textbook: https://www.opengeomatics.ca/topology-and-geocoding.html#fig:7-3d-alpha-hull-conifer.

(ref:7-3d-alpha-hull-conifer-caption2) 3D convex alpha hull for a coniferous tree (Abies lasiocarpa). The alpha hull was generated using alpha = 0.1. Data collected by Yangqian (Frederick) Qi with a terrestrial laser scanner in British Columbia, Canada. Pickell, CC-BY-SA-4.0.
...
knitr::include_graphics("images/07-3d-alpha-hull-conifer.png")
} else { fig_cap = "(ref:7-3d-alpha-hull-conifer-caption2)"
...
3. Define topology.
4. How are Thiessen Polygons related to the Delaunay Triangulation?
5. How do you interpret a Voronoi diagram? Give an example for how you could use this layer to solve a spatial problem.
6. What are some uses of a multipart geometry?
