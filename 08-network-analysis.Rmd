...
#### Least Cost Paths
The notion of cost let us compare different routes. It relates to the necessary distance (either physically or logically) to go from a certain node to another.
...
map %>% 
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolygons(data=intersections, color=~colorQuantile("viridis", closeness, reverse=TRUE)(closeness)) %>%
...
pal <- colorNumeric(palette="viridis", domain=segments$betweenness, n=6, reverse=TRUE)
...
<p id="box-text"></p>
Are green spaces evenly accessible throughout the whole city? Which parks are closer to the whole city? Centrality analysis can be used to answer these questions.

First we need to run the Closeness for the street network of the City of Vancouver. The network information can be downloaded for free from [OpenStreetMap](https://www.openstreetmap.org/).

This is the result of running the Closeness centrality analysis for the whole city. Now we can overlay Parks & Green spaces data from the [City of Vancouver Open Data portal](https://opendata.vancouver.ca/explore/dataset/parks-polygon-representation/information/) and get the average closeness of each green space.
