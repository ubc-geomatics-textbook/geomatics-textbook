...
**Geostatistics** uses the metrics based on statistical tools that are used to characterize the distribution of an event across the geographical region of interest (Getis et al., 2004).In context of spatial data, it is important to understand what is occurring where. Sometimes there is a detailed and precise information of an event across the geographical area such that a continuous map can be built out of that information. While, most of the time , only discrete and unrepresentative information on the spatial event is collected, which doesnot allow to create a precise continuous surface of the event over entire geographic area of interest. In such scenario, researchers should focus on two things; 

1) How can researchers best utilize the available discrete information to represent the event across entire area using the appropriate sampling strategies. 

2) Once a good sampling strategy is employed, researchers should be able to estimate the occurrence of the event across the unsampled region in the entirety of study area using proper spatial estimation technique.

This chapter introduces some basic ideas on different types of **sampling** strategies in spatial context. In addition, this chapter also introduces various type of spatial statistics that are being used in **estimating** the occurrence of events in the unsampled locations. 
...
In classical statistics, the variance is assumed to be totally random when samples are drawn from a defined sampling space [@Jacquez] and are assumed to come from one distribution [@Steel and Torrie 1990]. The inference about the population is the based on the comparison of a test statistic calculated for a sample to the distribution of the statistic under the null hypothesis for the reference distribution [@Jacquez]. The significance level of a test statistics from the sample is compared with the critical value of test statistics obtained by repeatedly reordering the data at random proportion [@Jacquez]. Interpretation of this significance level on the classical model: if less than or equal to the a level (usually 5%) the null hypothesis of ‘no pattern’ is rejected [@Jacquez].

While in geostatistics, the variance is assumed to be partly random and each point in the field represents a sample from some distribution. However,the distribution at any one point may differ completely from that at all other points in its shape, mean, and variance. The distribution of differences in sample values separated by a specified distance is assumed to be the same over the entire field. In geostatistics, if the sample values are similar (also know as **spatial autocorrelation**) given that they are in proximity to each other, the random variance of the distribution of differences in sample values is relatively small. If the sample values doesnot show spatial autocorrelation, the variance is larger. In geostatistics, the **semi-variance**, i.e., half of the variance is used to measure the similarity between points at a given distance apart. Using the semivarince, a graph of semivariance versus distance is constructed, know as **semi-variogram**. Thus, in geostatistics, there are two stages: 1) semivariogram construction and 2) incorporating semi-variogram to estimate the values at unsampled locations using various spatial methods (Clark, 1980). We will get into details about semivariogram construction and spatial methods in the later sections of this chapter. 
...
Before getting into the details about different types of sampling. Lets get familiar with some sampling key terms and their definitions.
...
For example, when we take another sample of 60 as in **Figure 10.2**, we can see that different sampling units are being selected from what we obtained in **Figure 10.1.**, this represent the equal probability of each sampling unit being selected.
...
##### Step1: We will build the neighborhood based on the queens case using package "spdep" in R using poly2nb function and plot the the linkage {-}
...
##### Step 1: We will calculate all the neighborhood list exactly how we did for Moran's I. In our final step we will use geary funtion from "spdep" package to calculate the value of Geary's C {-}
...
##### Step 2: Create polygons throughout the study area where interpolation is to be done and rasterize the polygongs {-}
...
##### Step 3: Nearest neighbor with five unsampled points to be interpolated at a time usign gstat package and plotting the results {-}
...
##### Step 2: Create an empty grid over Fort Saint johns TSA, where n is the total number of cells over which interpolation is to be done {-}
...
proj4string(grd) <- proj4string(dta)
```
##### Step 2: Lets calculate the mean of the basal area so that we can use it in our simple kriging, which assumes that mean is a known constant over entire domain {-}
...
##### Step 3: Lets start by fitting the semivariogram model for the basal area and see which model best fit the data. {-}
...
##### Step 5: We will use beta = 95.73, as we know the assume mean is a known constant over the domain for simple kriging  {-}
...
For classical statistics tests,**spatial autocorrelation** is problematic as ordinary least square (regression) or analysis of variance (ANOVA) assumes that observations are independent in space and time. However, geostatistical data violates the assumptions of independence, and using regression and ANOVA might inflate the significance of t and F statistics, when, in fact, they may not be significant at all [@Meng]. In that case one should try to improve the regression model by adding important **auxiliary** (independent variables that are associated or important in predicting the variables of interest) and incorporating the spatial autocorrelation structure using spatial regression model. We will show two examples of spatial regression model in this section using our familiar YSm data for Fort Saint Johns TSA. 
...
#### Case Study {-}
:::
For this case study, we will use ground plot data from Young stand monitoring (YSM) program data [@ProvinceofBC2018] for Fort saint Johns timber supply area (TSA) in the province of British Columbia, Canada.Fort saint johns is divided into 6 blocks respectively **Figure 10.17**. There is a total of 108 YSM plots used in this study **Figure 10.17**. The total basal area (m2/ha) is our variable of interest or response variable in this study. For each of the YSM plot the total basal area was calculated by adding the basal area for all trees within the plot. We will use the auxillary variables such as stems per hectare, elevation, site index, and slope percent 
<p id="box-text">
</p>
...
Spatial lag models assume that the spatial autocorrelation only exist in the response variable or the variable of interest.

##### Steps in fitting spatial lag model 

### Spatial error model 

Spatial error models assume that the spatial autocorrelation exists in the residuals or the error term of the regression equation.
