...
Light Detection and Ranging (LiDAR), Surface Models, LiDAR Metrics, Area-Based Approach (ABA), Individual Tree Crown Detection (ITD).
...
Unlike passive remote sensing technologies, LiDAR has the advantage of being able penetrate forest canopies; this means that we are able to detect the ground, and also characterize both vertical and horizontal vegetation structure. Since the data is extremely accurate, surface models developed using LiDAR are used in many fields where man-mane and natural environments need to be mapped. LiDAR derivatives are used in hazard assessment, forestry, agriculture, wet area mapping, and geologic mapping. In this chapter, we will discuss the use of LiDAR primarily in a forestry context, where raw data and derivatives are used for volume and biomass estimation, as well as individual tree crown analyses.

-   **MORE? - link to some example free/open case studies?** Examples include OPEN articles, but can also be linked as recommended readings
    -   @Goodbody2021
    -   @Jaboyedoff2012
    -   @Hamraz2017
...
LiDAR point data needs to be synced with positioning data in order to know exactly where the point is in space. To do this, a very accurate GPS clock is used to time stamp the laser scanning data [@UFGeomatics-FortLauderdale2016a]. Accurate clocks are imperative for producing accurate point clouds; one nanosecond (i.e. one billionth of a second) corresponds to a 30 cm travel distance, as seen in Figure \@ref(fig:15-LiDAR-Discrete-Full-Waveform) [@UFGeomatics-FortLauderdale2016]!
...
LiDAR point clouds require post-processing to be useful. Often, point clouds contain noise that needs to be removed, while the points need to be classified to produce derivatives. Once a point cloud has been cleaned and classified, we can create a variety of products to describe the Earth's surface, as well as describe vertical characteristics of our point clouds. The most important points to identify in a point cloud are ground and non-ground returns; these points are crucial for deriving digital elevation models, digital surface models, and canopy height models.

- **MORE?**

### Bare Earth Elevation

Digital elevation models (DEMs) are raster products that represent height of the Earth's surface above a reference such as sea level [@White2013]. Using a classified point cloud, we can isolate the points that are categorized as ground (`Classification == 2`) and create a raster that represents the ground. DEMs are also called digital terrain models (DTMs), although a DTM can include vector features that represent natural features (**Check that this language aligns with other chapters**). DEMs have a variety of uses, from terrain modeling for watersheds, to road design [@Roussel2021]. In addition to being used on their own, DEMs can be used to normalize point clouds; this is the process of subtracting the ground elevation from a point cloud in order to place the points on a flat plane, so that each point represents the height above the ground, not absolute height. To create a DEM, various interpolation techniques are available - these techniques are covered in more detail in **Chapter X**, but common algorithms include inverse distance weighting (IDW), kriging, and k-nearest neighbour (KNN).
...
Digital surface models (DSMs) represent the height of surface features above the ground, relative to the same reference as DEMs [@White2013]. In contrast to a DEM, the DSM captures the natural and built features of the environment, and can be thought of as a table cloth placed over a scene. When a point cloud is normalized by subtracting a DEM from a DSM, the derived surface can be called a canopy height model (CHM), and represents the height of the canopy above ground level [@White2013]. While the elevation values are different, both surfaces can be derived using the same algorithms [@Roussel2021].
...
For this case study, we will be using a clipped .las file from the 2018 open [LiDAR dataset](https://opendata.vancouver.ca/explore/dataset/lidar-2018/information/){target="_blank"} of the City of Vancouver and UBC Endowment Lands in British Columbia (we randomly selected a 200 x 200 meter portion of the '4840E_54550N' tile using CloudCompare), and the `lidR` package in R [@CityofVancouver, @Roussel2020]. The script to process this data is included HERE [where to include script?], and you can use the `lidR` [book](https://jean-romain.github.io/lidRbook/){target="_blank"} to get a more in depth understanding of the functions we apply below [@Roussel2021].
...
fig_cap <- paste("A segmented point cloud using the algorithm based on @Dalponte2016.")
...
LiDAR is an active remote sensing technology that requires three components; a laser scanner, a GNSS, and an IMU. Time of flight principles are used to produce 3D point clouds, which can be used monitor vegetation structure, stream properties, and topography. Point cloud derivatives such using ITD or ABA approaches can help us analyze different aspects of a forest or landscape at a variety of different resolutions.
...
3.  What are typical LiDAR wavelengths?\
4.  How is a DEM created? What steps are needed?

## Recommended Readings {.unnumbered}

"A best practices guide for generating forest inventory attributes from airborne laser scanning data using an area-based approach. Information Report FI-X-010" - [@White2013]

"Airborne laser scanning for quantifying criteria and indicators of sustainable forest management in Canada" - [@Goodbody2021]

```{r include=FALSE, echo=FALSE}
knitr::write_bib(c(
  .packages(), 'bookdown', 'knitr', 'rmarkdown', 'htmlwidgets', 'webshot', 'DT',
  'miniUI', 'tufte', 'servr', 'citr', 'rticles', 'leaflet', 'raster', 'sf', 'sp',
  'htmlwidgets', 'rgdal', 'leafem', 'geojsonio'), 'packages.bib')
```
